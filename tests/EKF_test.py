...
from util_tloc import ext_feat_and_label,distance

def EKFTest(SysModel, test_input, test_target, dataset_name, sinerio, prior_flag, encoder_type, model_encoder_trained,d):
    # N_T = test_target.size()[0]
    # test_feat, test_target = ext_feat_and_label(test_input)
    serving_bs = test_input[['Lon','Lat']].iloc[0,:]
    N_T = 1
    ...
    EKF = ExtendedKalmanFilter(SysModel,dataset_name, prior_flag,encoder_type, model_encoder_trained)
    ...
    for j in range(0, N_T):
        print(j)
        start = time.time()
        EKF.GenerateSequence(test_input, d, test_target, EKF.T_test) #j sample (all trajectory is getting to EKF pipeline)

        pred = EKF.x
        pred[:, 0] += serving_bs[0]
        pred[:, 1] += serving_bs[1]
        error = [distance(pt1, pt2) for pt1, pt2 in zip(pred, test_input[['Longitude','Latitude']].values)]
        error = sorted(error)
        print("EKF Test Loss: {}".format(np.median(error)))
