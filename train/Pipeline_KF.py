...
class Pipeline_KF:
    def __init__(self, Time, path_KNetLatent, dataset_name, sinerio, fix_encoder_flag, encoder_type, ssModel, d, warm_start_flag):
        super().__init__()
    ...
    def get_seq_knet_output(self,observation_seq , target, prior_flag):
        ...
        if self.dataset_name == "tloc":
            vobs = observation_seq[["vLon", "vLat"]]
            vobs_first_index = vobs.index[0]
            observation_seq, obs_target = ext_feat_and_label(observation_seq)
        ...
        state_prev = self.ssModel.m1x_0

        # prediction of raf encoder
        if self.encoder_type == "RaF":
            z_encoder_output = self.encoder.predict(observation_seq)

        # prediction of NN encoder
        # elif self.encoder_type == "torchNN":
        ...
        for t in range(0, length_seq):
            ...
            x_out[:,t]= self.model(z_encoder_output[:, t].clone(),(vLon,vLat)).clone()
