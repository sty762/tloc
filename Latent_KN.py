...
class KalmanNetLatentNN(torch.nn.Module):
    ...
    def step_prior(self):
        ...
        elif self.dataset_name == "tloc":
            self.F_matrix = self.f_function(self.m1x_posterior.float())
            self.m1x_prior = torch.matmul(self.F_matrix, self.m1x_posterior.float())
        else:
    ...
    def forward(self, y, vobs = None):
        ...
        if self.dataset_name == "tloc":
            if vobs:
                self.m1x_prior[2] = vobs[0]
                self.m1x_prior[3] = vobs[1]
